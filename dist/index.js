...
var ResourceManager = class extends BaseClass {
  constructor() {
    super();
  }
  static getMovieClip(scFile, export_name, a3 = 0) {
    let exportNamePtr;
    if (typeof export_name === "string") {
      exportNamePtr = export_name.ptr();
    } else {
      exportNamePtr = export_name;
    }
    return new NativeFunction(this.add(9581544), "pointer", ["pointer", "pointer", "int"])(scFile.ptr(), exportNamePtr, a3);
  }
  static getSupercellSWF(swfName) {
    return new NativeFunction(this.add(12031132), "pointer", ["pointer", "int"])(swfName.ptr(), 0);
  }
};
...
// agent/Classes/flash.ts
var MovieClip = class extends BaseClass {
  constructor() {
    super();
  }
  static getMovieClipByName(movieClipPtr, name) {
    let namePtr;
    ...
    return new NativeFunction(this.add(12221088), "pointer", ["pointer", "pointer"])(movieClipPtr, namePtr);
  }
  static getTextFieldByName(movieClipPtr, name) {
    let namePtr;
    if (typeof name === "string") {
      namePtr = name.ptr();
    } else {
    ...
    return new NativeFunction(this.add(12221696), "pointer", ["pointer", "pointer"])(movieClipPtr, namePtr);
  }
  static gotoAndStopFrameIndex(movieClipPtr, frameIndex) {
    return new NativeFunction(this.add(12217964), "pointer", ["pointer", "int"])(movieClipPtr, frameIndex);
  }
};
var TextField = class extends BaseClass {
  constructor() {
    super();
  }
  static setText(textFieldPtr, text) {
    return new NativeFunction(this.add(12407920), "pointer", ["pointer", "pointer"])(textFieldPtr, typeof text === "string" ? text.scptr() : text);
  }
};

// agent/Classes/GameMain.ts
var GameMain = class extends BaseClass {
  constructor() {
    super();
  }
  static loadAsset(path, a2 = 0) {
    return new NativeFunction(this.add(4841756), "pointer", ["pointer", "int"])(path.scptr(), a2);
  }
};

// agent/SupercellSWF/movieclip.ts
var MovieClip2 = class {
...
var SupercellSWF = class _SupercellSWF {
  ...
  static getSWF(swfName) {
    return new _SupercellSWF(ResourceManager.getSupercellSWF(swfName));
  }
  getSWF() {
    return this.swf;
  }
...
var SCEditor = class {
  ...
  loadUI() {
    GameMain.loadAsset("sc/sce_ui.sc");
  ...
  disableOriginalButtons(top) {
    ["button_load", "button_exit", "button_misc", "button_exports"].forEach((name) => {
      MovieClip.getMovieClipByName(top, name.ptr()).add(8).writeInt(0);
    });
  }
  createCoreButtons() {
    const top = this.getTopMC();
    this.buttons.set("load", this.createButton(MovieClip.getMovieClipByName(top, "button_load".ptr()), 500));
    this.buttons.set("exit", this.createButton(MovieClip.getMovieClipByName(top, "button_exit".ptr()), 500));
    this.buttons.set("exports", this.createButton(MovieClip.getMovieClipByName(top, "button_exports".ptr()), 500));
  ...
  showDebug() {
    ...
    const text = [
      ...
      `Frames: ${MovieClip2.getTotalFrames(this.selectedClip)}
`,
    ...
    Interceptor.attach(this.base.add(12421880), {
      onEnter: (args) => {
        ...
        if (this.miscButtons.length > 0 && clicked.equals(this.miscButtons[0])) {
          ...
          if (this.timelineEnabled) {
            ...
            timelinesliderTask.setFunction(() => {
              ...
              if (MovieClip2.getCurrentFrame(this.selectedClip) == 0) {
              ...
              const moveBy = delta / MovieClip2.getTotalFrames(this.selectedClip);
              x += moveBy;
        ...
        if (this.playbackBtn != null && clicked.equals(this.playbackBtn) && this.timelineEnabled && this.selectedClip != null) {
          ...
          if (this.timelineIsPlaying) {
            MovieClip2.play(movieClip);
          } else {
            MovieClip2.pause(movieClip);
          }
        }
        if (this.miscButtons.length > 0 && clicked.equals(this.miscButtons[2])) {
          ...
          for (let i = 0; i < this.selectedFile.getMovieClipCount(); i++) {
            ...
            if (id == 8) {
              const x_mc = element;
              const MovieClip3 = new NativeFunction(base2.add(12207104), "pointer", ["pointer", "pointer", "int"])(x_mc, this.selectedFile.getSWF(), 0);
              MovieClip3.add(32).writeFloat(200);
              MovieClip3.add(36).writeFloat(200);
              Stage.addChild(MovieClip3);
            }
          }
        }
    ...
    if (!this.interceptorAttached) {
      ...
      Interceptor.attach(base2.add(12421880), {
        onEnter: (args) => {
          ...
          const clip = ResourceManager.getMovieClip(`sc/${this.selectedSCFile.replace("sc/", "")}`, uisc.getExportNameAt(index), 0);
          if (this.selectedClip) {
            new NativeFunction(base2.add(12285420), "pointer", ["pointer", "pointer"])(this.background, this.selectedClip);
          }
          new NativeFunction(base2.add(12284692), "pointer", ["pointer", "pointer"])(this.background, clip);
